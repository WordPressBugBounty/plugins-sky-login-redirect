...
if (!class_exists('ZenSettings\ZenSettings')) {
    ...
    class ZenSettings
    {
        ...
        private function __construct(
            ...
            $this->_parent_slug = $args['parent_slug'] ?? '';
         ...
         *
         * @param array $args      args
         * @param array $menu_icon menu_icon
         * @param array $tabs      tabs
         * @param array $icons     icons
         * @param array $features  features
         *
        ...
        public static function getInstance(
            ...
            if (self::$instance === null) {
                self::$instance = new self($args, $menu_icon, $tabs, $icons, $features);
            }
        ...
        public function init()
        {
            ...
            add_action('admin_menu', array($this, 'create_menu'), 9999);
        ...
        public function enqueue_scripts($hook)
        {
            ...
            // if parent_slug is filled in
            // then we are in a subdir and need to amend the slug to fit in
            if (!empty($this->_parent_slug)) {
                $this->_slug = $this->_parent_slug;
            }

            $plugin_pages = [
                "toplevel_page_{$this->_slug}",
                "{$this->_slug}_page_{$this->_slug}-settings",
            ...
            if ($hook == $plugin_pages[0] || $hook == $plugin_pages[1]) {
                ...
                $color_picker = "jQuery(document).ready(function($) { jQuery('.colorpicker').wpColorPicker();});";
                wp_add_inline_script('wp-color-picker', $color_picker);
        ...
        {
            // debug
            //error_log("Parent slug is: " . $this->_parent_slug);

            if (!empty($this->_parent_slug)) {
                //wp_die("Parent slug is: " . $this->_parent_slug);
                add_submenu_page(
                    $this->_parent_slug, // Parent slug
                    $this->_page_title,          // Page title
                    $this->_menu_title,          // Menu title
                    'manage_options',            // Capability
                    $this->_slug,                // Menu slug
                    [$this, $this->_callback]    // Callback function
                );
            } else {
                //wp_die("Creating menu page because parent slug is empty.");  // Debug line
                add_menu_page(
                    $this->_page_title,
                    $this->_menu_title,
                    'manage_options',
